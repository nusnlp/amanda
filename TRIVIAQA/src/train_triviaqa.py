...
def get_ans_string_single_post_pad_search_updated_wiki(context,
						       context_words,
						       ans_start_pred,
						       ans_end_pred, maxspan=15):
	ans_start = ans_start_pred[:len(context_words)]
...
def get_prediction_answer_strings_updated_wiki(tokenized_datapath,
					       textdatapath,
					       dev_pred_dict):
	textdata = load_data(textdatapath)['data']
	ctx_dict = {}
	...
	for paragraph in paragraphs:
		...
		for qa in paragraph['qas']:
			if qa['id'] in keys_list:
				...
				pred_dict[qa['id']] = get_ans_string_single_post_pad_search_updated_wiki(ctx_dict[qa['id']],
												    context_words,
												    ans_start_pred,
												    ans_end_pred)
...
def minibatch_generator(data, batch_size=10, maxwordlen=10, shuffling=True):
	qids = data.keys()
	if shuffling:
		shuffle(qids)
...
def minibatch_generator_withmaxctxlen(data, batch_size=60,
	...
	def get_updated_story_answer_start(story, astart,
				       alen, context_chars,
				       maxctxlen):
		if len(story) > maxctxlen:
			...
			if start_ind + alen <= maxctxlen:
			...
			elif start_ind + alen > maxctxlen/2 and len(story) - (start_ind + alen) > maxctxlen/2:
				...
				context_chars = context_chars[start_ind - maxctxlen/2 : start_ind] + \
					...
					context_chars[start_ind + alen : start_ind + alen + maxctxlen/2]
			elif start_ind + alen > maxctxlen/2 and len(story) - (start_ind + alen) <= maxctxlen/2:
				story = story[max(0, start_ind-(maxctxlen-(len(story)-start_ind-alen))):start_ind] + \
					story[start_ind:]
				astart = astart[max(0, start_ind-(maxctxlen-(len(astart)-start_ind-alen))):start_ind] + \
					astart[start_ind:]
				context_chars = context_chars[max(0, start_ind-(maxctxlen-(len(context_chars)-start_ind-alen))):start_ind] + \
	...
	while 1:
		for indx in range(0, num_batches + 1):
			...
			for key in keylist:
				...
				story, astart, context_chars = get_updated_story_answer_start(story, astart, alen,
											      context_chars, maxctxlen)
				assert len(story) == len(astart)
				start_ind = astart.index(1)
				ans_len = alen
				aend = [0] * len(astart)
				if start_ind + ans_len -1 < len(astart):
					aend[start_ind + ans_len - 1] = 1
				else:
					aend[start_ind] = 1
				qwh = data[key]['q_wh']
...
def predict_batchwise(data, model, batch_size=15, maxwordlen=10):
	qids = data.keys()
	num_samples = len(qids)
	num_batches = num_samples / batch_size
	pred_dict = {}
...
def data_debug(train_data, samples=1000):
	new_data = {}
	keys = train_data.keys()[:min(samples, len(train_data))]
	for key in keys:
...
if __name__ == "__main__":
	#
	parser = argparse.ArgumentParser(description='Training on TriviaQA dataset')
	# data inputs
	parser.add_argument('-indextr', '--indexed_train_json', type=str,
			    default='data/indexed_web-train.json',
			    help='Indexed train JSON file')
	parser.add_argument('-devjs', '--dev_json', type=str, default='squad-like-data/web-dev.json',
			    help='formatted dev JSON file')
	parser.add_argument('-tokdev', '--tok_dev_json', type=str,
			    default='data/tokenized-web-dev.json',
			    help='tokenized dev JSON file')
	parser.add_argument('-indexdev', '--indexed_dev_json', type=str,
			    default='data/indexed_web-dev.json',
			    help='Indexed dev JSON file')
	parser.add_argument('-rawdevjs', '--raw_dev_json', type=str, default='qa/web-dev.json',
			    help='raw dev JSON file')
	parser.add_argument('-mode', '--data_mode', type=str, default='web',
			    help='web/wiki triviaqa?')

	parser.add_argument('-id2c', '--id2char', type=str, default='prep-data/id2char_web.json',
	...
	parser.add_argument('-word_emb', '--embed_mat_path', type=str,
			    default='prep-data/embed_mat_web.npy',
	...
	parser.add_argument('-ne', '--num_epoch', type=int, default=15,
	...
	parser.add_argument('-exp', '--baseexp', type=str, default='exp-triviaqa-web',
	...
	for epoch in range(args.initial_epoch, args.num_epoch):
		...
		if args.pretrained_weightpath is not None and epoch == args.initial_epoch:
			...
			if args.data_mode == 'wiki' or 'wiki' in args.data_mode:
				prev_predictions = get_prediction_answer_strings_updated_wiki(processed_data,
										    textdatapath,
										    prev_dev_pred_dict)
				prev_predictions = convert_doc_level_pred_to_question_level(
					prev_predictions)
			else:
			...
			prev_stats = Evaluator.evaluate_triviaqa(key_to_ground_truth, prev_predictions)
			prev_best_em = prev_stats['exact_match']
			prev_best_f1 = prev_stats['f1']
			logging.info(json.dumps(prev_stats))
		...
		dev_pred_dict = predict_batchwise(dev_data, model,
						  ...
						  maxwordlen=args.maxwordlen)
		if args.data_mode == 'wiki' or 'wiki' in args.data_mode:
			predictions = get_prediction_answer_strings_updated_wiki(processed_data,
									    textdatapath,
									    dev_pred_dict)
			predictions = convert_doc_level_pred_to_question_level(predictions)
		else:

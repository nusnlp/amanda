...
def get_embed_mat(glove_file, id2word_file):
	...
	print "obtained pretrained values for " + str(count) + " words out of " + str(len(vocab_words)) + " words"
	return embed_mat


if __name__ == "__main__":
	#
	parser = argparse.ArgumentParser(description='Training on TriviaQA Web dataset')
	...
	parser.add_argument('-word_emb', '--embed_mat_path', type=str,
			    default='prep-data/embed_mat_web.npy', help='path of the word embed mat')
	...
	parser.add_argument('-id2w', '--id2word', type=str, default='prep-data/id2word_web.json',
			    help='id2word JSON file')
	parser.add_argument('-glv', '--glovefile', type=str, default=None,
			    help='path of the GloVe text file')

	# dump predictions
	parser.add_argument('-tdir', '--test_pred_dir', type=str, default='./',
	...
	v_predictions = get_prediction_answer_strings_updated(v_processed_data,
								       v_textdatapath,
								       v_dev_pred_dict)
	print "Extracting ground truth answers..."
    ...
    #
	# t_predictions = get_prediction_answer_strings_updated(t_processed_data,
	# 							       t_textdatapath,
	# 							       test_pred_dict)
	# with open(os.path.join(args.test_pred_dir, 'web_test_predictions.json'), 'w') as fp:

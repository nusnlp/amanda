...
if __name__ == "__main__":
	parser = argparse.ArgumentParser(description='Indexed data preparation for SearchQA from the tokenized data')
	parser.add_argument('-data', '--datafile', type=str, default='data/tokenized-train.json',
				help = 'tokenized data json file name')

	parser.add_argument('-id2w', '--id2wordf', type=str, default='prep-data/id2word.json',
				help='id2word vocab file name')

	parser.add_argument('-id2c', '--id2charf', type=str, default='prep-data/id2char.json',
                                help='id2char vocab file name')

	parser.add_argument('-wr', '--writef', type=str, default='data/train_indexed.json',
	...
	dp = DataPrep(data_file, vocab_file, char_vocab_file)
	cqa = dp.prep_data()
	print "Dumping..."

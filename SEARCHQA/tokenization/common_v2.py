...
def process_tokens_v2(temp_tokens):
	l = ["-", "\u2212", "\u2014", "\u2013", "/", "~", '"', "'", "\u201C", "\u2019", "\u201D", "\u2018", "\u00B0"]
	tokens = []
	for token in temp_tokens:
		for item in l:
			if item in token:
				items = token.split(item)
				join_item = ' ' + item + ' '
				token = join_item.join(items)
		tokens += token.split(' ')
...
def word_tokenize(sent):
	...
	new_tokens = []
	for token in tokens:
		if token != '.' and token[-1] in ['.']:  # normalize 1960. and 1960
			new_tokens.extend([token[:-1], '.'])
		# elif '-' in token: # split words like 'top-five' to 'top' and 'five'.
		#    new_tokens.extend(' - '.join(filter(lambda x: x, token.split('-'))).split(' '))
		# elif '\'' in token:
		#    new_tokens.extend(" ' ".join(filter(lambda x: x, token.split('\''))).split(' ')) # year '03.
		else:
			new_tokens.append(token)
	return process_tokens_v2(new_tokens)
